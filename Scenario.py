...
class Scenarios:
    ...
    def convert_pool_uncertainties_to_variables_uncertainties(self, ucertainties):
        transactions = list(self.X_pool[:, 0])
        ucertainties = list(np.around(ucertainties, decimals=2))

        all_concepts_uncertainties = dict(zip(transactions, ucertainties))

        concept_uncertainties = {}
        for sym in self.variables:
            concept_uncertainties.update({sym: all_concepts_uncertainties[int(str(sym).split('v')[-1])]})
